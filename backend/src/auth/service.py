...
async def authenticateUser(req_email :str, req_password :str, db :Session) -> schema.TokenSchema:
    #query if user with email exists
    user = db.query(model.User).filter(model.User.email == req_email).first()
